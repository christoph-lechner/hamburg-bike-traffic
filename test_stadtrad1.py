...
#
# The original URL does not define the sort order of the objects. As consequence as we navigate through the dataset using the "@iot.nextLink" (retrieving 100 objects at a time), we get some objects multiple times while not getting other objects even a single time
url = 'https://iot.hamburg.de/v1.0/Datastreams?$count=true&$orderBy=name&$filter=properties/serviceName%20eq%20%27HH_STA_StadtRad%27%20and%20properties/layerName%20eq%20%27Fahrraeder%27&$expand=Observations($select=phenomenonTime,result;$orderby=phenomenonTime%20desc;$top=10)'
...
while url:
    ...
    with open(f'out/log_{cntr}.txt','w') as fout:
